  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex justify-between items-start">
        <div>
          <h2 className="text-xl font-bold text-slate-900 dark:text-white">Application Tracker</h2>
          <p className="text-xs text-slate-600 dark:text-slate-400">Manage job responses, online assessments, and dynamic schedule timelines</p>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        <div className="glass-panel p-4 rounded-xl flex items-center justify-between">
          <div>
            ...
            <p className="text-2xl font-black text-slate-900 dark:text-white">{totalApplied}</p>
          </div>
        ...
        <form onSubmit={handleAddJob} className="glass-panel p-6 rounded-2xl grid grid-cols-1 md:grid-cols-3 gap-4 border border-blue-500/20">
          <div className="space-y-1">
            ...
            <input 
              ...
              className="w-full bg-slate-200/50 dark:bg-slate-950/60 border border-slate-300 dark:border-white/10 rounded-lg px-3 py-1.5 text-xs text-slate-900 dark:text-white focus:outline-none focus:border-neonBlue"
            />
          </div>

          <div className="space-y-1">
            <label className="block text-xs font-semibold text-slate-600 dark:text-slate-400">Target Role</label>
            <input 
          ...
          <div className="space-y-1">
            <label className="block text-xs font-semibold text-slate-600 dark:text-slate-400">Salary Package</label>
            <input 
          ...
          <div className="space-y-1">
            <label className="block text-xs font-semibold text-slate-600 dark:text-slate-400">Submission Stage</label>
            <select 
              value={stage} onChange={e => setStage(e.target.value)}
              className="w-full bg-slate-200/50 dark:bg-slate-950/60 border border-slate-300 dark:border-white/10 rounded-lg px-3 py-2 text-xs text-slate-900 dark:text-white focus:outline-none focus:border-neonBlue"
            >
              {STAGES.map((s, idx) => (
          ...
          <div className="space-y-1">
            <label className="block text-xs font-semibold text-slate-600 dark:text-slate-400">Deadline Date</label>
            <input 
          ...
          <div className="space-y-1">
            <label className="block text-xs font-semibold text-slate-600 dark:text-slate-400">Additional Notes</label>
            <input 
          ...
          <div className="md:col-span-3 flex justify-end gap-2 pt-2 border-t border-slate-200 dark:border-white/5">
            <button 
              type="button" onClick={() => setShowAddForm(false)}
              className="text-xs font-semibold text-slate-450 hover:text-slate-900 dark:text-white px-3 py-2 rounded transition-colors"
            >
              Cancel
            </button>
            <button 
      ...
      <div className="grid grid-cols-1 md:grid-cols-5 gap-4 overflow-x-auto pb-4">
        {STAGES.map((currentStage) => {
          ...
          return (
            <div key={currentStage} className="bg-slate-100/80 dark:bg-slate-900/35 border border-slate-200 dark:border-white/5 rounded-2xl p-4 space-y-4 shrink-0 min-w-[220px]">
              
              {/* Header column title */}
              <div className="flex justify-between items-center border-b border-slate-200 dark:border-white/5 pb-2">
                <span className="text-xs font-extrabold text-slate-700 dark:text-slate-300">{currentStage}</span>
              ...
              <div className="space-y-3 max-h-[50vh] overflow-y-auto pr-1">
                {stageJobs.map((job) => (
                  <div key={job._id} className="p-3 bg-slate-200/50 dark:bg-slate-950/50 border border-slate-200 dark:border-white/5 hover:border-blue-500/20 rounded-xl space-y-2 relative group transition-all duration-300">
                    
                    {/* Trash delete */}
                    <button 
                      onClick={() => handleDeleteJob(job._id)}
                      className="absolute top-2 right-2 p-1 bg-white dark:bg-slate-900 border border-slate-200 dark:border-white/5 rounded text-slate-500 hover:text-red-400 hover:border-red-500/20 opacity-0 group-hover:opacity-100 transition-opacity"
                    >
                      <Trash2 className="w-3.5 h-3.5" />
                    </button>

                    <div>
                      <span className="text-xs font-bold text-slate-800 dark:text-slate-200 block truncate pr-5">{job.company}</span>
                    ...
                    <div className="flex justify-between items-center pt-1 border-t border-slate-200 dark:border-white/5">
                      <span className="text-[9px] text-slate-600 dark:text-slate-400">{job.salary || 'Salary N/A'}</span>
                      
                      {currentStage !== 'Offer Extended' && currentStage !== 'Rejected' && (
                        <button
                          onClick={() => handleAdvanceStage(job._id, currentStage)}
                          className="p-1 bg-white dark:bg-slate-900 hover:bg-slate-50 dark:bg-slate-850 rounded border border-slate-200 dark:border-white/5 text-slate-600 dark:text-slate-400 hover:text-slate-900 dark:text-white transition-colors"
                          title="Advance stage"
                        >
