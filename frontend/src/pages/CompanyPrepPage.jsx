...
const CompanyPrepPage = ({ user, token, API_BASE, updateProfile }) => {
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Header */}
      <div>
        <h2 className="text-xl font-bold text-slate-900 dark:text-white">Company Prep Engine</h2>
        <p className="text-xs text-slate-600 dark:text-slate-400">Master hiring rounds, DSA topics, and core CS questions for top-tier companies</p>
      </div>
          ...
          <div className="lg:col-span-2 space-y-6">
            ...
            <div className="glass-panel p-6 rounded-2xl space-y-4 relative overflow-hidden">
              ...
              <div className="flex justify-between items-start z-10 relative">
                <div className="flex items-center gap-3">
                  <div className="w-12 h-12 bg-white dark:bg-slate-900 border border-slate-200 dark:border-white/5 rounded-xl flex items-center justify-center font-extrabold text-lg text-slate-700 dark:text-slate-300">
                    {selectedCompany[0]}
                  </div>
                  <div>
                    <h3 className="text-lg font-bold text-slate-900 dark:text-white">{selectedCompany}</h3>
                    <span className="text-[10px] text-slate-600 dark:text-slate-400 font-mono">Overview & Recruitment Path</span>
                  </div>
                </div>
              ...
              <div className="pt-2 border-t border-slate-200 dark:border-white/5 z-10 relative">
                <h4 className="text-xs font-semibold text-slate-700 dark:text-slate-300 mb-2">Hiring Pipeline Summary</h4>
                <p className="text-xs text-slate-600 dark:text-slate-400 leading-relaxed">{companyDetails.process}</p>
              </div>
            </div>
            ...
            <div className="glass-panel p-6 rounded-2xl space-y-4">
              <div className="flex items-center gap-2 border-b border-slate-200 dark:border-white/5 pb-3">
                ...
                <h4 className="text-sm font-semibold text-slate-900 dark:text-white">Specific Interview Stages</h4>
              </div>

              <div className="relative border-l border-slate-300 dark:border-white/10 ml-3 pl-5 space-y-5">
                {companyDetails.rounds && companyDetails.rounds.map((round, idx) => (
                  <div key={idx} className="relative">
                    ...
                    <span className="text-xs font-bold text-slate-800 dark:text-slate-200">Round {idx + 1}: {round}</span>
            ...
            <div className="glass-panel p-6 rounded-2xl space-y-4">
              <div className="flex items-center gap-2 border-b border-slate-200 dark:border-white/5 pb-3">
                ...
                <h4 className="text-sm font-semibold text-slate-900 dark:text-white">Recruitment FAQs</h4>
              </div>

              <div className="space-y-4">
                {companyDetails.faqs && companyDetails.faqs.map((faq, idx) => (
                  <div key={idx} className="space-y-1">
                    <span className="text-xs font-bold text-slate-700 dark:text-slate-300">Q: {faq.q}</span>
                    <p className="text-xs text-slate-600 dark:text-slate-400 leading-relaxed pl-4 border-l border-slate-200 dark:border-white/5">
                      {faq.a}
                    </p>
                  </div>
                ))}
          ...
          <div className="space-y-6">
            ...
            <div className="glass-panel p-6 rounded-2xl space-y-4">
              <div className="flex items-center gap-2 border-b border-slate-200 dark:border-white/5 pb-2">
                ...
                <h4 className="text-sm font-semibold text-slate-900 dark:text-white">DSA Algorithms Frequency</h4>
              </div>
            ...
            <div className="glass-panel p-6 rounded-2xl space-y-4">
              <div className="flex items-center gap-2 border-b border-slate-200 dark:border-white/5 pb-2">
                ...
                <h4 className="text-sm font-semibold text-slate-900 dark:text-white">Core Technical Subjects</h4>
              </div>
            ...
            <div className="glass-panel p-6 rounded-2xl space-y-4">
              <div className="flex items-center gap-2 border-b border-slate-200 dark:border-white/5 pb-2">
                ...
                <h4 className="text-sm font-semibold text-slate-900 dark:text-white">Sample Behavioral Prompts</h4>
              </div>
              <ul className="space-y-3">
                {companyDetails.behavioralQuestions && companyDetails.behavioralQuestions.map((q, idx) => (
                  <li key={idx} className="text-xs text-slate-600 dark:text-slate-400 leading-relaxed border-l border-rose-500/30 pl-3">
                    "{q}"
                  </li>
                ))}
              </ul>
