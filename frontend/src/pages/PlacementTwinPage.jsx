...
const PlacementTwinPage = ({ user, token, API_BASE }) => {
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Header */}
      <div>
        <h2 className="text-xl font-bold text-slate-900 dark:text-white">Placement Twin & AI Mentor</h2>
        <p className="text-xs text-slate-600 dark:text-slate-400">Consult your digital double matching capabilities to target roles and review next action sequences</p>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-5 gap-6">
        ...
        <div className="lg:col-span-2 space-y-6">
          <div className="glass-panel p-6 rounded-2xl space-y-5 relative overflow-hidden">
            ...
            <div className="flex items-center gap-3">
              <div className="w-10 h-10 rounded-full bg-gradient-to-tr from-purple-500 to-pink-500 flex items-center justify-center text-slate-900 dark:text-white border border-slate-300 dark:border-white/10">
                <Brain className="w-5 h-5" />
              </div>
              <div>
                <h3 className="text-sm font-bold text-slate-900 dark:text-white">Digital Twin Profile</h3>
            ...
            <div className="p-4 bg-slate-100/80 dark:bg-slate-900/40 border border-slate-200 dark:border-white/5 rounded-xl space-y-2">
            ...
            <div className="space-y-4 pt-2">
              <div className="space-y-2">
                <span className="text-xs font-bold text-slate-700 dark:text-slate-300">Audited Strengths</span>
              ...
              <div className="space-y-2">
                <span className="text-xs font-bold text-slate-700 dark:text-slate-300">Primary Growth Areas</span>
                <div className="flex flex-wrap gap-2">
        ...
        <div className="lg:col-span-3">
          <div className="glass-panel rounded-2xl flex flex-col h-[60vh] relative overflow-hidden">
            ...
            {/* Chat Header */}
            <div className="p-4 border-b border-slate-200 dark:border-white/5 flex items-center justify-between bg-slate-100/80 dark:bg-slate-900/30">
              <div className="flex items-center gap-2">
                <div className="w-2 h-2 rounded-full bg-emerald-400 animate-ping"></div>
                <span className="text-xs font-bold text-slate-800 dark:text-slate-200">AI Twin Consultant</span>
              </div>
            </div>
            ...
            <div className="flex-1 overflow-y-auto p-4 space-y-4">
                ...
                <div key={idx} className={`flex items-start gap-2.5 ${msg.sender === 'user' ? 'justify-end' : ''}`}>
                  ...
                  <div className={`p-3.5 rounded-xl text-xs max-w-[80%] leading-relaxed ${
                    msg.sender === 'user'
                      ...
                      : 'bg-slate-100/80 dark:bg-slate-900/60 border border-slate-200 dark:border-white/5 text-slate-250 rounded-tl-none'
                  }`}>
                    {msg.text}
                  </div>
              ...
              {sending && (
                <div className="flex items-start gap-2.5">
                  ...
                  <div className="p-3 bg-slate-100/80 dark:bg-slate-900/60 border border-slate-200 dark:border-white/5 rounded-xl text-xs text-slate-500 rounded-tl-none flex items-center gap-1.5">
            ...
            {/* Input Form */}
            <form onSubmit={handleSendMessage} className="p-4 border-t border-slate-200 dark:border-white/5 flex gap-2 bg-slate-100/80 dark:bg-slate-900/20">
              <input
                type="text"
                value={inputText}
                onChange={(e) => setInputText(e.target.value)}
                placeholder="Ask about TCS rounds, DSA goals, or resume keyword fixes..."
                className="flex-1 bg-slate-200/50 dark:bg-slate-950/60 border border-slate-300 dark:border-white/10 rounded-lg px-3 py-2 text-xs text-slate-900 dark:text-white focus:outline-none focus:border-neonBlue transition-colors"
                disabled={sending}
              />
              <button
                type="submit"
