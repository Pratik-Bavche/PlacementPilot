  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        ...
        {/* Welcome Card */}
        <div className="md:col-span-2 glass-panel p-6 rounded-2xl relative overflow-hidden flex items-center justify-between">
          ...
          <div className="space-y-3 z-10">
            ...
            <p className="text-sm text-slate-400 max-w-md">
              Your AI Mentor suggests preparing for <span className="text-white font-semibold">{user.targetCompany || 'Google'}</span> SDE interviews. Solve dynamic programming challenges and review your resume keywords to accelerate your readiness score.
            </p>
          ...
          <div className="hidden lg:flex flex-col items-center justify-center p-4 bg-slate-900/40 rounded-xl border border-white/5 w-36">
            <Flame className="w-10 h-10 text-orange-500 animate-pulse fill-orange-500/20" />
            <span className="text-2xl font-extrabold text-white mt-1">{user.streak || 1} Days</span>
        ...
        <div className="glass-panel p-6 rounded-2xl flex flex-col items-center justify-center text-center">
          <h3 className="text-sm font-semibold text-slate-400 mb-3">Overall Placement Readiness</h3>
          <div className="relative w-32 h-32 flex items-center justify-center">
            ...
            <div className="text-center z-10">
              <span className="text-3xl font-extrabold text-white glow-text-blue">{readinessVal}%</span>
              <p className="text-[9px] text-slate-500 font-bold uppercase tracking-wider mt-0.5">Readiness Score</p>
            </div>
          </div>
          <button 
            onClick={() => setTab('readiness')}
            className="mt-4 text-xs font-semibold text-neonBlue hover:text-white flex items-center gap-1 transition-colors"
          >
      ...
      <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-4">
        ...
        <div className="glass-panel p-4 rounded-xl space-y-1 bg-slate-900/30">
          <div className="flex justify-between items-start">
            <span className="text-[10px] text-slate-400 font-semibold uppercase tracking-wider">Resume Score</span>
            <FileText className="w-4 h-4 text-sky-400" />
          </div>
          <p className="text-xl font-bold text-white">{stats ? stats.resumeScore : 82}/100</p>
        ...
        <div className="glass-panel p-4 rounded-xl space-y-1 bg-slate-900/30">
          <div className="flex justify-between items-start">
            <span className="text-[10px] text-slate-400 font-semibold uppercase tracking-wider">Communication</span>
            <BrainCircuit className="w-4 h-4 text-violet-400" />
          </div>
          <p className="text-xl font-bold text-white">{stats ? stats.communicationScore : 78}/100</p>
          <div className="w-full bg-slate-800 h-1.5 rounded-full overflow-hidden">
        ...
        <div className="glass-panel p-4 rounded-xl space-y-1 bg-slate-900/30">
          <div className="flex justify-between items-start">
            <span className="text-[10px] text-slate-400 font-semibold uppercase tracking-wider">DSA Progress</span>
            <BookOpen className="w-4 h-4 text-emerald-400" />
          </div>
          <p className="text-xl font-bold text-white">{stats ? stats.dsaScore : 80}/100</p>
          <div className="w-full bg-slate-800 h-1.5 rounded-full overflow-hidden">
        ...
        <div className="glass-panel p-4 rounded-xl space-y-1 bg-slate-900/30">
          <div className="flex justify-between items-start">
            <span className="text-[10px] text-slate-400 font-semibold uppercase tracking-wider">GitHub Health</span>
            <TrendingUp className="w-4 h-4 text-amber-400" />
          </div>
          <p className="text-xl font-bold text-white">{stats ? stats.githubScore : 75}/100</p>
          <div className="w-full bg-slate-800 h-1.5 rounded-full overflow-hidden">
        ...
        <div className="glass-panel p-4 rounded-xl space-y-1 bg-slate-900/30">
          <div className="flex justify-between items-start">
            <span className="text-[10px] text-slate-400 font-semibold uppercase tracking-wider">Project Score</span>
            <Award className="w-4 h-4 text-rose-400" />
          </div>
          <p className="text-xl font-bold text-white">{stats ? stats.projectScore : 70}/100</p>
          <div className="w-full bg-slate-800 h-1.5 rounded-full overflow-hidden">
        ...
        <div className="glass-panel p-4 rounded-xl space-y-1 bg-slate-900/30">
          <div className="flex justify-between items-start">
            <span className="text-[10px] text-slate-400 font-semibold uppercase tracking-wider">Target company</span>
            <Briefcase className="w-4 h-4 text-indigo-400" />
          </div>
          <p className="text-lg font-bold text-white truncate">{user.targetCompany || 'Google'}</p>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2 glass-panel p-6 rounded-2xl space-y-4">
          <div className="flex justify-between items-center border-b border-white/5 pb-3">
            <div className="flex items-center gap-2">
              ...
              <h3 className="text-base font-semibold text-white">Daily Learning Tasks</h3>
            </div>
            <button 
              onClick={() => setTab('roadmap')}
        ...
        <div className="glass-panel p-6 rounded-2xl space-y-4">
          <div className="border-b border-white/5 pb-3">
            <h3 className="text-base font-semibold text-white">Recommended Companies</h3>
          ...
          <div className="space-y-3.5">
            {stats && stats.predictions ? (
              stats.predictions.map((p, idx) => (
                <div key={idx} className="flex justify-between items-center">
                  <div className="flex items-center gap-2.5">
                    <div className="w-8 h-8 rounded-lg bg-slate-850 flex items-center justify-center border border-white/5">
                      <span className="text-xs font-extrabold text-slate-300">{p.company[0]}</span>
                    </div>
                    <div>
                      <span className="text-xs font-bold text-slate-200">{p.company}</span>
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <button 
                      ...
                      className="p-1 bg-slate-900 hover:bg-slate-800 rounded border border-white/5 text-slate-400 hover:text-white transition-colors"
                    >
              ...
              ['Google', 'Amazon', 'Microsoft', 'TCS', 'Infosys'].map((company, idx) => (
                <div key={idx} className="flex justify-between items-center">
                  <div className="flex items-center gap-2">
                    <span className="text-xs text-slate-300 font-semibold">{company}</span>
                  </div>
